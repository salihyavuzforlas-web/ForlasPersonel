...
export const mapPersonnelTarget = (row: any): PersonnelTarget => ({
  id: row.id,
  userId: row.user_id || row.userId,
  userName: row.user_name || row.userName || 'Personel',
  title: row.title || '',
  targetType: (row.target_type ||
    row.targetType ||
    row.raw?.target_type ||
    row.raw?.targetType ||
    PersonnelTargetType.SALES) as PersonnelTargetType,
  productName: row.product_name || row.productName || row.raw?.product_name || row.raw?.productName || '',
  description: row.description || row.raw?.description || '',
  metricUnit: row.metric_unit || row.metricUnit || row.raw?.metric_unit || 'Adet',
  targetValue: Number(row.target_value ?? row.targetValue ?? 0),
  currentValue:
    row.current_value != null
      ? Number(row.current_value)
      : row.raw?.current_value != null
      ? Number(row.raw.current_value)
      : undefined,
  startDate: row.start_date || row.startDate,
  endDate: row.end_date || row.endDate,
  isActive: !!row.is_active,
  createdById: row.created_by_id || row.createdById,
  createdByName: row.created_by_name || row.createdByName || 'Yönetici',
  createdAt: row.created_at || row.createdAt || Date.now(),
  updatedAt: row.updated_at || row.updatedAt || Date.now(),
});

export const mapPersonnelSale = (row: any): PersonnelSale => ({
  id: row.id,
  userId: row.user_id || row.userId,
  userName: row.user_name || row.userName || 'Personel',
  saleDate: row.sale_date || row.saleDate,
  amount: Number(row.amount || 0),
  note: row.note || row.raw?.note || '',
  createdAt: row.created_at || row.createdAt || Date.now(),
  updatedAt: row.updated_at || row.updatedAt || Date.now(),
});
