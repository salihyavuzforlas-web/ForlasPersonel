...
import { buildRouteLocationOptions, getCityFromLocationLabel } from '../data/routeLocations';
import SalesTopPerformersCard from './SalesTopPerformersCard';
...
const RoutePlannerView: React.FC<RoutePlannerViewProps> = ({ user }) => {
  ...
  const routeLocationOptions = useMemo(() => buildRouteLocationOptions(cityOptions), [cityOptions]);

  const routeMetrics = useMemo(() => {
    const fromCity = getCityByName(getCityFromLocationLabel(formData.startLocation));
    const toCity = getCityByName(getCityFromLocationLabel(formData.endLocation));
          ...
          <div className="bg-white rounded-2xl border border-slate-200 w-full max-w-2xl shadow-2xl overflow-hidden">
            ...
            <form onSubmit={handleSubmit} className="p-4 grid grid-cols-1 md:grid-cols-2 gap-3">
              <datalist id="route-location-options">
                {routeLocationOptions.map((locationValue) => (
              ...
              <input
                ...
                placeholder="Başlangıç (Şehir / İlçe / Sanayi)"
                className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none"
              />
              <input
                required
                ...
                placeholder="Bitiş (Şehir / İlçe / Sanayi)"
                className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none"
              />
              <input
